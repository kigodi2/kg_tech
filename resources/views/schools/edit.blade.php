    ...
    <div class="bg-white p-6 rounded shadow">
        ...
        <form method="POST" action="/schools/{{ $school->id }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="code" value="{{ $school->code }}" class="w-full border p-2 rounded" required>
                @error('code')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror
            </div>
            
            <div class="mb-4">
                <label class="block text-gray-700 mb-2">Name</label>
                <input type="text" name="name" value="{{ $school->name }}" class="w-full border p-2 rounded" required>
            ...
            <div class="mb-4">
                ...
                <select name="school_type" class="w-full border p-2 rounded" required>
                    ...
                    <option value="BOTH" {{ $school->school_type == 'BOTH' ? 'selected' : '' }}>Both</option>
                </select>
            </div>
            
            <div class="mb-4">
                <label class="block text-gray-700 mb-2">Address</label>
                <input type="text" name="address" value="{{ $school->address }}" class="w-full border p-2 rounded">
            </div>
            
            <div class="mb-4">
                <label class="block text-gray-700 mb-2">Phone</label>
                <input type="text" name="phone" value="{{ $school->phone }}" class="w-full border p-2 rounded">
            </div>
            
            <div class="mb-4">
                <label class="block text-gray-700 mb-2">Email</label>
                <input type="email" name="email" value="{{ $school->email }}" class="w-full border p-2 rounded">
            </div>
            
            <div class="mb-4">
                <label class="block text-gray-700 mb-2">Principal Name</label>
                <input type="text" name="principal_name" value="{{ $school->principal_name }}" class="w-full border p-2 rounded">
            </div>
