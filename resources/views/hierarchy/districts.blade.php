...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4 mb-4">
            <a href="{{ route('hierarchy.regions') }}" class="text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-2">
                <i class="fas fa-arrow-left"></i> Back to Regions
            </a>
        </div>
        <h1 class="text-3xl font-bold text-gray-900">{{ $region->name }} - Districts</h1>
        <p class="mt-2 text-gray-600">Select a district to view schools</p>
    </div>

    <!-- 4-Column Grid -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        @forelse($districts as $index => $district)
            <a href="{{ route('hierarchy.schools', $district->id) }}"
               ...
               style="background: linear-gradient(135deg, 
                ...
                <div class="relative p-1 h-11 flex flex-col items-center justify-center text-white">
                    <div class="text-center">
                        <p class="text-base font-bold uppercase tracking-wider">{{ $district->name }}</p>
                    </div>
                </div>
            </a>
        @empty
            <div class="col-span-4 text-center py-12">
                <p class="text-gray-500 text-lg">No districts available in this region</p>
            </div>
        @endforelse
    </div>
</div>
