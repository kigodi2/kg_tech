<div x-data="importModal()" x-show="showModal" x-cloak @open-import-modal.window="open($event.detail.type, $event.detail.context)">
  <div class="fixed inset-0 z-[1000]">
    <div class="absolute inset-0 bg-black/50" @click="close()"></div>
    <div class="absolute inset-0 flex items-start justify-center overflow-y-auto p-4">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-5xl mt-10" @keydown.escape.window="close()">
        <div class="border-b px-5 py-4 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <h3 class="text-lg font-semibold text-gray-800" x-text="title"></h3>
            <span class="text-xs px-2 py-0.5 rounded-full" :class="importType==='single_csv'?'bg-blue-100 text-blue-700':importType==='school_zip'?'bg-purple-100 text-purple-700':'bg-indigo-100 text-indigo-700'" x-text="badge"></span>
          </div>
          <button class="text-gray-500 hover:text-gray-700" @click="close()"><i class="fas fa-times"></i></button>
        </div>

        <!-- Stepper -->
        <div class="px-5 pt-4">
          <div class="flex items-center gap-4 text-sm">
            <div class="flex items-center gap-2" :class="step>=1?'text-blue-600':'text-gray-400'"><span class="w-6 h-6 inline-flex items-center justify-center rounded-full border" :class="step>=1?'border-blue-600':'border-gray-300'">1</span> Upload</div>
            <div class="h-px flex-1 bg-gray-200"></div>
            <div class="flex items-center gap-2" :class="step>=2?'text-blue-600':'text-gray-400'"><span class="w-6 h-6 inline-flex items-center justify-center rounded-full border" :class="step>=2?'border-blue-600':'border-gray-300'">2</span> Validate</div>
            <div class="h-px flex-1 bg-gray-200"></div>
            <div class="flex items-center gap-2" :class="step>=3?'text-blue-600':'text-gray-400'"><span class="w-6 h-6 inline-flex items-center justify-center rounded-full border" :class="step>=3?'border-blue-600':'border-gray-300'">3</span> Commit</div>
          </div>
        </div>

        <div class="p-5 space-y-5">
          <!-- Step 1: Upload -->
          <div x-show="step===1" class="space-y-4">
            <!-- Visibility hint -->
            <div class="bg-amber-50 border border-amber-200 rounded p-3 flex items-start gap-2 text-sm">
              <i class="fas fa-info-circle text-amber-600 mt-0.5"></i>
              <div class="text-amber-800">
                <strong>Visibility:</strong> Imported marks appear in Reports only after Approval and Locking via Moderation & Review.
              </div>
            </div>

            <div class="border-2 border-dashed rounded-lg p-6 text-center cursor-pointer hover:border-blue-500" @click="$refs.file.click()">
              <i class="fas fa-cloud-upload-alt text-3xl text-gray-400 mb-2"></i>
              <p class="text-gray-700">Click to choose a file or drag & drop</p>
              <p class="text-gray-500 text-xs" x-text="importType==='single_csv' ? 'CSV/TXT up to 5MB' : 'ZIP up to 200MB' "></p>
              <input x-ref="file" type="file" :accept="importType==='single_csv'?'.csv,.txt':'.zip'" class="hidden" @change="onFileChange($event)">
            </div>
            <template x-if="file">
              <div class="bg-gray-50 border rounded p-3 flex items-center justify-between">
                <div class="text-sm text-gray-700">
                  <span class="font-semibold" x-text="file?.name"></span>
                  <span class="text-gray-500"> • </span>
                  <span class="text-gray-500" x-text="prettySize(file?.size)"></span>
                </div>
                <button class="text-red-600 text-sm" @click="clearFile()"><i class="fas fa-trash-alt"></i> Remove</button>
              </div>
            </template>
            <div class="flex justify-end gap-2">
              <button class="px-4 py-2 bg-blue-600 text-white rounded disabled:opacity-50" :disabled="!file || isValidating" @click="validate()">
                <span x-show="!isValidating"><i class="fas fa-check mr-1"></i> Validate</span>
                <span x-show="isValidating"><i class="fas fa-spinner fa-spin mr-1"></i> Validating...</span>
              </button>
            </div>
          </div>

          <!-- Step 2: Validate Results -->
          <div x-show="step===2" class="space-y-4">
            <!-- Status badge -->
            <div class="flex items-center gap-3">
              <span class="text-sm font-semibold px-3 py-1 rounded-full"
                :class="{
                  'bg-green-100 text-green-800': validationReport?.status === 'completed',
                  'bg-yellow-100 text-yellow-800': validationReport?.status === 'partial',
                  'bg-red-100 text-red-800': validationReport?.status === 'failed',
                }"
                x-text="(validationReport?.status || 'unknown').toUpperCase()"></span>
              <span class="text-xs text-gray-500" x-show="validationReport?.run_id">Run #<span x-text="validationReport?.run_id"></span></span>
            </div>

            <div class="grid grid-cols-4 gap-3">
              <div class="bg-blue-50 border border-blue-200 rounded p-3">
                <p class="text-xs text-gray-600">Total Rows</p>
                <p class="text-2xl font-bold text-blue-700" x-text="validationReport?.totals?.total_rows ?? 0"></p>
              </div>
              <div class="bg-green-50 border border-green-200 rounded p-3">
                <p class="text-xs text-gray-600">Valid</p>
                <p class="text-2xl font-bold text-green-700" x-text="validationReport?.totals?.valid_rows ?? 0"></p>
              </div>
              <div class="bg-red-50 border border-red-200 rounded p-3">
                <p class="text-xs text-gray-600">Invalid</p>
                <p class="text-2xl font-bold text-red-700" x-text="validationReport?.totals?.invalid_rows ?? 0"></p>
              </div>
              <div class="bg-yellow-50 border border-yellow-200 rounded p-3">
                <p class="text-xs text-gray-600">Warnings</p>
                <p class="text-2xl font-bold text-yellow-700" x-text="validationReport?.totals?.warnings ?? 0"></p>
              </div>
            </div>

            <!-- Structured errors table -->
            <template x-if="(validationReport?.errors?.length||0) > 0">
              <div class="border rounded">
                <div class="px-3 py-2 bg-red-50 border-b text-red-700 font-semibold text-sm flex items-center justify-between">
                  <span>Errors & Warnings</span>
                  <button class="text-xs bg-red-600 text-white px-2 py-1 rounded hover:bg-red-700" @click="downloadErrorCsv()">
                    <i class="fas fa-download mr-1"></i> Download Error CSV
                  </button>
                </div>
                <div class="max-h-56 overflow-auto">
                  <table class="w-full text-sm">
                    <thead class="bg-gray-50 sticky top-0">
                      <tr>
                        <th class="px-3 py-1 text-left">Row</th>
                        <th class="px-3 py-1 text-left">Index</th>
                        <th class="px-3 py-1 text-left">Field</th>
                        <th class="px-3 py-1 text-left">Message</th>
                        <th class="px-3 py-1 text-left">Code</th>
                        <th class="px-3 py-1 text-left">Severity</th>
                      </tr>
                    </thead>
                    <tbody>
                      <template x-for="e in validationReport.errors.slice(0, 25)" :key="`${e.row}-${e.index_number}-${e.field}-${e.code}`">
                        <tr class="border-t">
                          <td class="px-3 py-1" x-text="e.row"></td>
                          <td class="px-3 py-1" x-text="e.index_number"></td>
                          <td class="px-3 py-1" x-text="e.field"></td>
                          <td class="px-3 py-1" x-text="e.message"></td>
                          <td class="px-3 py-1 text-xs text-gray-500" x-text="e.code"></td>
                          <td class="px-3 py-1">
                            <span class="text-xs px-1.5 py-0.5 rounded-full"
                              :class="e.severity==='warning'?'bg-yellow-100 text-yellow-700':'bg-red-100 text-red-700'"
                              x-text="e.severity"></span>
                          </td>
                        </tr>
                      </template>
                    </tbody>
                  </table>
                </div>
                <div class="px-3 py-1 bg-gray-50 border-t text-xs text-gray-500" x-show="(validationReport?.errors?.length||0) > 25">
                  Showing first 25 of <span x-text="validationReport?.errors?.length"></span> issues. Download CSV for full report.
                </div>
              </div>
            </template>

            <!-- Preview rows -->
            <template x-if="(validationReport?.preview?.length||0) > 0">
              <div class="border rounded">
                <div class="px-3 py-2 bg-gray-50 border-b text-gray-700 font-semibold text-sm">Preview (first 20 rows)</div>
                <div class="max-h-64 overflow-auto">
                  <table class="w-full text-sm">
                    <thead class="bg-white sticky top-0">
                      <tr>
                        <th class="px-3 py-1 text-left">Row</th>
                        <th class="px-3 py-1 text-left">Index</th>
                        <th class="px-3 py-1 text-left">Status</th>
                        <th class="px-3 py-1 text-left">Messages</th>
                      </tr>
                    </thead>
                    <tbody>
                      <template x-for="p in validationReport.preview" :key="p.row">
                        <tr class="border-t">
                          <td class="px-3 py-1" x-text="p.row"></td>
                          <td class="px-3 py-1" x-text="p.index_number"></td>
                          <td class="px-3 py-1" :class="p.has_errors?'text-red-600':'text-green-600'" x-text="p.has_errors?'Invalid':'Valid'"></td>
                          <td class="px-3 py-1 text-xs text-gray-600">
                            <template x-for="m in (p.messages||[])" :key="m"><div x-text="m"></div></template>
                          </td>
                        </tr>
                      </template>
                    </tbody>
                  </table>
                </div>
              </div>
            </template>

            <div class="flex justify-between items-center">
              <button class="px-4 py-2 bg-gray-200 rounded" @click="step=1">Back</button>
              <div class="flex gap-2">
                <button class="px-4 py-2 bg-green-600 text-white rounded disabled:opacity-50" :disabled="!validationReport?.can_commit || isCommitting" @click="commit()">
                  <span x-show="!isCommitting"><i class="fas fa-play mr-1"></i> Commit</span>
                  <span x-show="isCommitting"><i class="fas fa-spinner fa-spin mr-1"></i> Committing...</span>
                </button>
              </div>
            </div>
          </div>

          <!-- Step 3: Commit/Progress -->
          <div x-show="step===3" class="space-y-4">
            <template x-if="importType==='single_csv'">
              <div class="space-y-3">
                <div class="bg-green-50 border border-green-200 rounded p-3" x-show="commitReport?.success">
                  <div class="font-semibold text-green-800"><i class="fas fa-check-circle mr-1"></i> Commit Successful</div>
                  <div class="text-sm text-green-700" x-text="commitReport?.message"></div>
                  <div class="text-xs text-green-600 mt-1" x-show="commitReport?.run_id">Import Run #<span x-text="commitReport?.run_id"></span></div>
                </div>
                <template x-if="(commitReport?.totals?.invalid_rows||0) > 0">
                  <div class="bg-yellow-50 border border-yellow-200 rounded p-3">
                    <div class="font-semibold text-yellow-800">Some Rows Had Errors</div>
                    <div class="text-sm text-yellow-700">
                      <span x-text="commitReport?.totals?.invalid_rows"></span> rows had validation errors.
                      <a :href="commitReport?.links?.error_report_url" class="underline font-medium" target="_blank">Download Error Report (CSV)</a>
                    </div>
                  </div>
                </template>
                <div class="bg-amber-50 border border-amber-200 rounded p-2 text-xs text-amber-800">
                  <i class="fas fa-info-circle mr-1"></i> Imported marks are <strong>PENDING REVIEW</strong>. They will appear in Reports after moderation approval and locking.
                </div>
                <div class="flex gap-2">
                  <a class="px-4 py-2 bg-blue-600 text-white rounded" :href="commitReport?.links?.batch_details_url" target="_blank">View Batch</a>
                  <a class="px-4 py-2 bg-red-600 text-white rounded" :href="commitReport?.links?.error_report_url" target="_blank" x-show="(commitReport?.totals?.invalid_rows||0) > 0"><i class="fas fa-download mr-1"></i> Errors CSV</a>
                </div>
              </div>
            </template>

            <template x-if="importType!=='single_csv'">
              <div class="space-y-4">
                <!-- Progress bar -->
                <div>
                  <div class="flex justify-between text-sm mb-1">
                    <span class="text-gray-700">Overall Progress</span>
                    <span class="font-semibold text-blue-700" x-text="(progress?.progress_percentage||0)+'%'"></span>
                  </div>
                  <div class="w-full h-3 bg-gray-200 rounded">
                    <div class="h-3 bg-blue-600 rounded" :style="'width:'+(progress?.progress_percentage||0)+'%'"></div>
                  </div>
                </div>
                <!-- Files/Schools status list -->
                <div class="max-h-64 overflow-auto space-y-2">
                  <template x-for="(item, idx) in (progress?.files || progress?.schools || [])" :key="idx">
                    <div class="border rounded p-2 flex justify-between items-center bg-gray-50">
                      <div class="flex-1 min-w-0 mr-2">
                        <div class="font-medium text-gray-800 truncate" x-text="item.subject_code || ((item.school_code||'-')+' - '+(item.school_name||item.school_code||'Unknown'))"></div>
                        <div class="text-xs text-gray-600" x-text="(item.rows_success||item.successful_candidates||0)+' / '+(item.rows_total||item.total_candidates||0)+' rows'"></div>
                        <div x-show="item.message" class="text-xs text-red-600 mt-0.5" x-text="item.message"></div>
                      </div>
                      <div class="text-sm font-semibold whitespace-nowrap" :class="item.status==='failed'?'text-red-600':item.status==='skipped'?'text-orange-600':item.status==='success'?'text-green-600':'text-blue-600'" x-text="(item.status||'processing').toUpperCase()"></div>
                    </div>
                  </template>
                </div>
                <div x-show="progress?.status !== 'completed' && progress?.status !== 'failed'" class="text-xs text-gray-500"><i class="fas fa-sync-alt fa-spin mr-1"></i> Auto-refreshing every 2s...</div>
                <div x-show="progress?.status === 'completed'" class="bg-green-50 border border-green-200 rounded p-3">
                  <div class="font-semibold text-green-800"><i class="fas fa-check-circle mr-1"></i> Import Completed Successfully</div>
                  <div class="text-sm text-green-700 mt-1">All files have been processed and marks imported.</div>
                </div>
                <div x-show="progress?.status === 'failed'" class="bg-red-50 border border-red-200 rounded p-3">
                  <div class="font-semibold text-red-800"><i class="fas fa-times-circle mr-1"></i> Import Failed</div>
                  <div class="text-sm text-red-700 mt-1">Some files failed to process. Check the details above.</div>
                </div>
                <!-- Error CSV download for ZIP runs -->
                <div x-show="commitReport?.run_id" class="flex gap-2">
                  <a class="px-3 py-1.5 bg-red-600 text-white rounded text-sm" :href="'/api/mark-entry/acsee/import/runs/'+commitReport?.run_id+'/errors.csv'" target="_blank" x-show="(commitReport?.run_id)">
                    <i class="fas fa-download mr-1"></i> Download Error Report (CSV)
                  </a>
                </div>
                <div class="bg-amber-50 border border-amber-200 rounded p-2 text-xs text-amber-800">
                  <i class="fas fa-info-circle mr-1"></i> Imported marks are <strong>PENDING REVIEW</strong>. They will appear in Reports after moderation approval and locking.
                </div>
              </div>
            </template>

            <div class="flex justify-end">
              <button class="px-4 py-2 bg-gray-700 text-white rounded" @click="close()">Close</button>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>

  <script>
    function importModal(){
      return {
        showModal:false, step:1, importType:'single_csv', title:'Upload Marks', badge:'',
        file:null, isValidating:false, isCommitting:false,
        validationReport:null, commitReport:null, progress:null, poller:null,
        context:{ exam_year:null, school_id:null, subject_id:null, district_id:null },
        open(type, ctx){ this.importType=type; this.title = type==='single_csv'?'Single Subject CSV':'Bulk ZIP Import'; this.badge = type==='single_csv'?'Single CSV':(type==='school_zip'?'School ZIP':'District ZIP'); this.context = { ...this.context, ...ctx }; this.showModal=true; this.step=1; this.file=null; this.validationReport=null; this.commitReport=null; this.progress=null; },
        close(){ this.stopPolling(); this.showModal=false; },
        onFileChange(e){ this.file=e.target.files[0]||null; },
        clearFile(){ this.file=null; this.$refs.file.value=''; },
        prettySize(s){ if(!s) return '0 B'; const u=['B','KB','MB','GB']; let i=0; while(s>1024&&i<u.length-1){s/=1024;i++} return s.toFixed(1)+' '+u[i]; },
        async validate(){
          if(!this.file) return; this.isValidating=true; try{
            const fd=new FormData(); fd.append('file', this.file);
            if(this.importType==='single_csv'){
              fd.append('exam_year', this.context.exam_year);
              fd.append('school_id', this.context.school_id);
              fd.append('subject_id', this.context.subject_id);
              const r=await fetch('/mark-entry/acsee/single/validate', { method:'POST', body:fd, headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')} });
              const j=await r.json(); if(!r.ok){ throw new Error(j.message||'Validate failed'); } this.validationReport=j; this.step=2;
            } else if(this.importType==='school_zip'){
              fd.append('exam_year_id', this.context.exam_year_id||'');
              fd.append('school_id', this.context.school_id||'');
              const r=await fetch('/mark-entry/acsee/bulk/school/validate-zip', { method:'POST', body:fd, headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')} });
              const j=await r.json(); if(!r.ok){ throw new Error(j.message||'Validate failed'); } this.validationReport=j; this.step=2;
            } else {
              fd.append('exam_year_id', this.context.exam_year_id||'');
              fd.append('district_id', this.context.district_id||'');
              const r=await fetch('/mark-entry/acsee/bulk/district/validate-zip', { method:'POST', body:fd, headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')} });
              const j=await r.json(); if(!r.ok){ throw new Error(j.message||'Validate failed'); } this.validationReport=j; this.step=2;
            }
          } catch(e){ alert(e.message); } finally{ this.isValidating=false; }
        },
        async commit(){
          this.isCommitting=true; try{
            if(this.importType==='single_csv'){
              const fd=new FormData(); fd.append('file', this.file); fd.append('exam_year', this.context.exam_year); fd.append('school_id', this.context.school_id); fd.append('subject_id', this.context.subject_id);
              const r=await fetch('/mark-entry/acsee/single/commit', { method:'POST', body:fd, headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')} });
              const j=await r.json(); if(!r.ok){ throw new Error(j.message||'Commit failed'); } this.commitReport=j; this.step=3;
            } else if(this.importType==='school_zip'){
              const fd=new FormData(); fd.append('school_id', this.context.school_id); fd.append('exam_year_id', this.context.exam_year_id);
              const r=await fetch('/mark-entry/acsee/bulk/school/commit-zip', { method:'POST', body:fd, headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')} });
              const j=await r.json(); if(!r.ok){ throw new Error(j.message||'Commit failed'); } this.commitReport=j; this.step=3;
              this.progress = { status:'completed', progress_percentage:100, files: j.files||[] };
            } else {
              const fd=new FormData(); fd.append('district_id', this.context.district_id); fd.append('exam_year_id', this.context.exam_year_id);
              const r=await fetch('/mark-entry/acsee/bulk/district/commit-zip', { method:'POST', body:fd, headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')} });
              const j=await r.json(); if(!r.ok && !j.schools){ throw new Error(j.message||'Commit failed'); } this.commitReport=j; this.step=3;
              const commitStatus = (j.success === false) ? 'failed' : 'completed';
              this.progress = { status:commitStatus, progress_percentage:100, schools: j.schools||[] };
            }
          } catch(e){ alert(e.message); } finally{ this.isCommitting=false; }
        },
        startPolling(id){ this.stopPolling(); if(!id) return; this.poller=setInterval(async()=>{ try{ const r=await fetch(`/mark-entry/acsee/bulk/${id}/progress`); const j=await r.json(); if(r.ok){ this.progress=j.progress||j; if((this.progress?.status||'')==='completed'||(this.progress?.status||'')==='failed'){ this.stopPolling(); } } }catch(_e){} }, 2000); },
        stopPolling(){ if(this.poller){ clearInterval(this.poller); this.poller=null; } },
        downloadErrorCsv(){
          if(!this.validationReport?.run_id) {
            // Fallback: download as JSON if no run_id
            const blob = new Blob([JSON.stringify(this.validationReport,null,2)], {type:'application/json'});
            const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='validation-report.json'; a.click(); URL.revokeObjectURL(url);
            return;
          }
          window.open('/api/mark-entry/acsee/import/runs/'+this.validationReport.run_id+'/errors.csv', '_blank');
        },
      }
    }
  </script>
</div>
