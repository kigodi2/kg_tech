                         ...
                         <div x-show="regionOpen" class="absolute top-full left-0 right-0 bg-white border border-t-0 border-gray-300 z-30 rounded-none flex flex-col">
                             ...
                             <div class="max-h-64 overflow-y-auto">
                                 ...
                                 <template x-for="region in regions.filter(r => r.name.toLowerCase().includes(regionSearch.toLowerCase()))" :key="region.id">
                                     <div 
                                         @click="filterRegion = region.id; regionOpen = false; filterDistricts(); filterSchools()"
                         ...
                         <div x-show="districtOpen" class="absolute top-full left-0 right-0 bg-white border border-t-0 border-gray-300 z-30 rounded-none flex flex-col">
                             ...
                             <div class="max-h-64 overflow-y-auto">
                                 ...
                                 <template x-for="district in filteredDistricts.filter(d => d.name.toLowerCase().includes(districtSearch.toLowerCase()))" :key="district.id">
                                     <div 
                                         @click="filterDistrict = district.id; districtOpen = false; filterSchools()"
            ...
            <table x-show="!loading" class="w-full">
                    ...
                    <tr>
                        <th class="px-4 py-3 text-left">
                            <input 
                                ...
                                :checked="selectedItems.size === filteredSchools.length && filteredSchools.length > 0"
                                class="w-4 h-4 cursor-pointer"
                            >
                        </th>
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700 uppercase">Number</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700 uppercase">School Name</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700 uppercase">Ownership</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    <template x-for="school in filteredSchools" :key="school.id">
                        <tr class="hover:bg-blue-100 transition-colors" :class="selectedItems.has(school.id) ? 'bg-blue-50' : ''">
                            <td class="px-4 py-4 text-left">
                                <input 
                                    type="checkbox" 
                                    :checked="selectedItems.has(school.id)"
                                    @change="toggleSelect(school.id)"
                                    class="w-4 h-4 cursor-pointer"
                                >
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-800 font-mono" x-text="school.code"></td>
                            <td class="px-6 py-4 text-sm text-gray-800 font-medium" x-text="school.name"></td>
                            <td class="px-6 py-4 text-sm">
                                <span :class="school.ownership === 'GOVERNMENT' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'" class="px-3 py-1 rounded-full text-xs font-semibold" x-text="school.ownership || '-'"></span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600" x-text="school.district_name || '-'"></td>
                            <td class="px-6 py-4 text-sm text-gray-600" x-text="school.region_name || '-'"></td>
                            <td class="px-6 py-4 text-sm text-gray-600 text-center" x-text="school.candidates_count || 0"></td>
                    ...
                    <template x-if="filteredSchools.length === 0 && !loading">
                        <tr>
                            <td colspan="8" class="px-6 py-8 text-center text-gray-500">
                                No schools found. <button @click="openAddModal()" class="text-blue-600 hover:underline font-medium">Add one now</button>
                            </td>
                        </tr>
                    </template>
                </tbody>
        ...
        >
            <div class="registration-modal-shell max-w-2xl" x-transition>
                ...
                <form x-show="!viewModalOpen" @submit.prevent="saveSchool()" class="registration-modal-body space-y-5">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Ownership *</label>
                        <select 
                            x-model="formData.ownership"
                            required
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                        >
                    ...
                    <div>
                        ...
                        >
                            ...
                            <template x-for="district in districtsByRegion" :key="district.id">
...
function schoolManager() {
    return {
        ...
        loadDistrictsByRegion() {
            ...
            this.districtsByRegion = this.districts.filter(d => d.region_id == this.formData.region_id);
        },
        ...
        async saveSchool() {
            try {
                // Validate required fields
                 if (!this.formData.code || !this.formData.name || !this.formData.ownership || !this.formData.region_id || !this.formData.district_id) {
                ...
                this.showMessage('Error saving school: ' + error.message, 'error');
            }
        },
        ...
        toggleSelectAll() {
            if (this.selectedItems.size === this.filteredSchools.length) {
                this.selectedItems.clear();
            } else {
                this.filteredSchools.forEach(school => this.selectedItems.add(school.id));
            }
        },
        ...
        downloadTemplate() {
            const headers = ['Code', 'Name', 'Ownership', 'Region ID', 'District ID'].join(',');
            const csv = headers;
            const blob = new Blob([csv], { type: 'text/csv' });
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `schools_template_${Date.now()}.csv`;
        ...
        exportCSV() {
            ...
            const rows = this.filteredSchools.map(s => 
                [s.code, s.name, s.ownership || '', s.district_name || '', s.region_name || ''].map(v => `"${v}"`).join(',')
            );
        ...
        async importCSV(event) {
            ...
            try {
                ...
                if (response.ok) {
                    let message = `Imported ${data.count} school(s) successfully`;
