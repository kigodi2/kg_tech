...
class PsleZonalwiseRemainingEvaluationsTest extends TestCase
{
    use RefreshDatabase;
    ...
    private DistrictCouncil $taboraCouncil;
    ...
    protected function setUp(): void
    {
        ...
        $this->taboraCouncil = DistrictCouncil::create([
            'region_id' => $this->tabora->id,
            'code' => 'TBC1',
        ...
        $this->taboraSchool = School::factory()->create([
            ...
            'council_id' => $this->taboraCouncil->id,
        ...
        $this->singidaSchool = School::factory()->create([
            ...
            'council_id' => $this->singidaCouncil->id,
    ...
    public function test_admin_can_view_all_remaining_zonal_evaluations(): void
    {
        $evaluations = [
            'best-ten-councils' => 'ZONAL BEST TEN (10) COUNCILS',
            'least-ten-councils' => 'ZONAL LEAST TEN (10) COUNCILS',
            'best-ten-schools' => 'ZONAL BEST TEN (10) SCHOOLS',
            'least-ten-schools' => 'ZONAL LEAST TEN (10) SCHOOLS',
            'ownership-result-evaluation' => 'ZONAL OWNERSHIP RESULT EVALUATION',
            'subjectwise-result-evaluation' => 'ZONAL SUBJECTWISE RESULT EVALUATION',
        ];

        foreach ($evaluations as $key => $label) {
            $response = $this->actingAs($this->admin)->get("/evaluations/psle/zonalwise/evaluation/{$key}");
            $response->assertStatus(200);
            $response->assertSee($label);
        }
    }

    public function test_admin_can_export_all_remaining_zonal_evaluations_pdf(): void
    {
        $evaluations = [
            'schoolwise',
            'best-ten-councils',
        ...
        foreach ($evaluations as $key) {
            $response = $this->actingAs($this->admin)->get("/evaluations/psle/zonalwise/evaluation/{$key}/export/pdf");
            $response->assertStatus(200);
