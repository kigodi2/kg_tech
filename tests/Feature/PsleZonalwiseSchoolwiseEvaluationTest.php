...
class PsleZonalwiseSchoolwiseEvaluationTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_admin_can_view_zonal_schoolwise_evaluation(): void
    {
        $response = $this->actingAs($this->admin)->get('/evaluations/psle/zonalwise/evaluation/schoolwise');

        $response->assertStatus(200);
        $response->assertSee('ZONAL SCHOOLWISE EVALUATION');
        $response->assertSee('TABORA PRIMARY');
        $response->assertSee('SINGIDA PRIMARY');
