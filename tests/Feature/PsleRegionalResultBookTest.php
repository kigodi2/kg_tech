...
class PsleRegionalResultBookTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function createSnapshotAndPayloads(bool $published = true, bool $hasActiveCorrection = false): int
    {
        ...
        // Seed ready precalculated payloads
        $payloads = [
            'general' => [
                'rows' => [
                    [
    ...
    public function test_admin_can_view_result_book_preview_page(): void
    {
        $this->createSnapshotAndPayloads(published: true);

        $response = $this->actingAs($this->admin)
            ->get("/evaluations/psle/regionalwise/{$this->region->id}/result-book");
    ...
    public function test_admin_can_override_settings_and_see_them_in_view(): void
    {
        ...
        $response = $this->actingAs($this->admin)
            ->get("/evaluations/psle/regionalwise/{$this->region->id}/result-book?reo_name=Ndg.+Salum+Mzee&marking_center=Tabora+Boys+Secondary");
