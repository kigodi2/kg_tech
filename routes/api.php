...
Route::post('/candidates/import', function (Request $request) {
    ...
    while (($row = fgetcsv($handle)) !== false) {
        ...
        try {
            ...
            // Normalize combination and resolve FK
            $comboCode = $combination ? strtoupper(trim($combination)) : null;
            $combo = $comboCode ? \App\Models\Combination::where('code', $comboCode)->first() : null;
            ...
            if (!$candidate) {
                // Create new candidate and persist combination fields
                $attrs['candidate_id'] = $candidateId;
                $attrs['combination'] = $comboCode;
                $attrs['combination_id'] = $combo?->id ?? null;
                $candidate = Candidate::create($attrs);

                // Integrity guard
                if ($comboCode && ($candidate->combination !== $comboCode || $candidate->combination_id !== ($combo?->id ?? null))) {
                    throw new \Exception("Combination mismatch for {$candidateId}: CSV='{$comboCode}' Saved='{$candidate->combination}'");
                }
            } else {
                // Existing candidate: only overwrite combination when in replace mode
                if ($mode === 'replace') {
                    $attrs['combination'] = $comboCode;
                    $attrs['combination_id'] = $combo?->id ?? null;
                }
                ...
                // Integrity guard for replaced combination
                if ($mode === 'replace' && $comboCode && ($candidate->combination !== $comboCode || $candidate->combination_id !== ($combo?->id ?? null))) {
                    throw new \Exception("Combination mismatch for {$candidateId} during replace: CSV='{$comboCode}' Saved='{$candidate->combination}'");
                }
            }
...
// NOTE: Bulk import routes are defined in web.php (under /api/bulk-import/*)
// to ensure session middleware is available for ZIP file handling.
// Do NOT duplicate them here — api.php routes lack session support.
